    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-orange-50">
                <tr>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-orange-900 uppercase tracking-wider">
                        Candidate
                    </th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-orange-900 uppercase tracking-wider">
                        Email
                    ...
                    <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-orange-900 uppercase tracking-wider">
                        Actions
                    </th>
                </tr>
            </thead>
                                ...
                                <div class="flex-shrink-0 h-10 w-10">
                                    ...
                                    @else
                                        <div class="h-10 w-10 rounded-full bg-orange-200 flex items-center justify-center">
                                            <span class="text-orange-700 font-semibold">{{ substr($candidate->prenom, 0, 1) }}{{ substr($candidate->nom, 0, 1) }}</span>
                                        </div>
                                    @endif
                                </div>
                                <div class="ml-4">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex justify-center items-center space-x-2">
                                ...
                                <form action="{{ route('admin.approve', $candidate->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center" onclick="return confirm('Approuver cette candidate ?')" title="Approuver">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Bouton Rejeter -->
                                <form action="{{ route('admin.reject', $candidate->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center" onclick="return confirm('Rejeter cette candidate ?')" title="Rejeter">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
@else
    <div class="text-center py-12">
        ...
        <h3 class="mt-2 text-sm font-medium text-gray-900">Aucune candidate en attente</h3>
